 ...
 */
export function createStepMessage(step: number, total: number, stepName: string, message: string): string {
 ...
 */
export function getContextualHint(action: 'transaction_saved' | 'account_created' | 'stats_viewed'): string {
  const hints = {
...
export function getTimeoutMessage(): string {
  return '⏱ Время ожидания истекло. Операция отменена.\n\n' +
